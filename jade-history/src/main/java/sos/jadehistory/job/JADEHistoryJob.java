...
public class JADEHistoryJob extends JobSchedulerJobAdapter {
    ...
    public long importFile(final Variable_set parameters) throws Exception {
        ...
        try {
            ...
            if (filePrefixLocal.toLowerCase().indexOf("-class=") != -1 || filePrefixLocal.toLowerCase().indexOf("-conn-str=") > -1) {
                ...
                sql = new StringBuffer("select \"POSITION\",\"FILE_SIZE\" from " + JADEHistory.TABLE_FILES_POSITIONS + " ")
                    .append("where \"LOCAL_FILENAME\" = '" + JADEHistory.getNormalizedField(getConnection(), localFilename, LENGTH255) + "'");
                try {
                ...
                for (int p = 0; p < positionRepeatCountLocal; p++) {
                    ...
                    if (recordPos != null && !recordPos.isEmpty()) {
                        ...
                        this.getLogger().debug1("not found file position for \"" + localFilename + "\" in database : try in " + positionRepeatIntervalLocal
                                + "s again");
                    }
                    if (foundPosition) {
                        break;
            ...
            if (foundPosition && position < recordFoundCount) {
                try {
                    sql = new StringBuffer("update " + JADEHistory.TABLE_FILES_POSITIONS + " ").append("set \"FILE_SIZE\" = " + importFileSize + ", ")
                        .append("    \"POSITION\" = " + recordFoundCount + " ").append("where \"LOCAL_FILENAME\" = '"
    ...
    private boolean importLine(final LinkedHashMap<String, String> hstRecordFields, final LinkedHashMap<String, String> phshRecordCustomFields,
        ...
        try {
            ...
            sql.append("select \"ID\" ").append("from " + JADEHistory.TABLE_FILES + " ").append("where \"MANDATOR\" = '" + mandator + AND)
                .append("       \"SOURCE_HOST\" = '" + sourceHost + AND).append("       \"SOURCE_HOST_IP\" = '" + sourceHostIp + AND)
                .append("       \"SOURCE_DIR\" = '" + sourceDir + AND).append("       \"SOURCE_FILENAME\" = '" + sourceFilename + AND)
                .append("       \"SOURCE_USER\" = '" + sourceUser + AND).append("       \"MD5\" = '" + md5 + "'");
