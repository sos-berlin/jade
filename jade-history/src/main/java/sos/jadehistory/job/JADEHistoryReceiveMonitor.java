...
public boolean spooler_process_after(final boolean arg0) throws Exception {
  ...
  private void fillPosition(final SOSConnection conn,String host,String remoteDir,String file) throws Exception{
    ...
    try{
      ...
      sql = new StringBuffer("select \"LOCAL_FILENAME\" from "+JADEHistory.TABLE_FILES_POSITIONS+ " ")
          .append("where \"HOST\" = '"+JADEHistory.getNormalizedField(conn,host,128)+"' and ")
          .append("   \"REMOTE_DIR\" = '"+JADEHistory.getNormalizedField(conn,remoteDir,255)+"' and ")
          .append("   \"REMOTE_FILENAME\" = '"+JADEHistory.getNormalizedField(conn,remoteFilename,255)+"'");
      ...
      if(lastLocalFileName == null || lastLocalFileName.length() == 0){
        ...
        sql = new StringBuffer("update "+JADEHistory.TABLE_FILES_POSITIONS+ " ")
            .append("set \"LOCAL_FILENAME\" = '"+JADEHistory.getNormalizedField(conn,localFilename,255)+"' ")
        .append("where \"LOCAL_FILENAME\" = '"+JADEHistory.getNormalizedField(conn,lastLocalFileName,255)+"'");
