...
public class SOSDataExchangeEngineMain extends I18NBase implements JSJobUtilities {
	...
	private void Execute(final String[] pstrArgs) {
		...
		try {
			...
			@SuppressWarnings("unused") String strLog4jPropertyFileName = objO.log4jPropertyFileName.Value();
	...
	@SuppressWarnings("unused") private void setExitCode() {
		setExitCode(99);
	}
	...
	@I18NMessages(
					value = { @I18NMessage("SOSDataExchange - Main routine started ..."), //
			@I18NMessage(
							value = "SOSDataExchange - Main",
							locale = "en_UK", //
							explanation = "SOSDataExchange - Main" //
			), //
			@I18NMessage(
							value = "SOSDataExchange - Kommandozeilenprogram startet ....",
							locale = "de", //
							explanation = "SOSDataExchange - Main" //
			)		//
					},
	...
	@I18NMessages(
					value = { @I18NMessage("%1$s: Error occurred ...: %2$s, exit-code 99 raised"), //
			@I18NMessage(
							value = "%1$s: Error occurred ...: %2$s",
							locale = "en_UK", //
							explanation = "%1$s: Error occurred ...: %2$s" //
			), //
			@I18NMessage(
							value = "%1$s: Fehler aufgetreten: %2$s, Programm wird mit Exit-Code 99 beendet.",
							locale = "de", //
							explanation = "%1$s: Error occurred ...: %2$s" //
			)		//
					},
	...
	@I18NMessages(
					value = { @I18NMessage("%1$s - ended without errorsended without errors"), //
			@I18NMessage(
							value = "%1$s - ended without errors",
							locale = "en_UK", //
							explanation = "%1$s - ended without errorsended without errors" //
			), //
			@I18NMessage(
							value = "%1$s - Programm wurde ohne Fehler beendet",
							locale = "de", //
							explanation = "%1$s - ended without errorsended without errors" //
			)		//
					},
	...
	@I18NMessages(
					value = { @I18NMessage("%1$s - terminated with exit-code %2$d"), //
			@I18NMessage(
							value = "%1$s - terminated with exit-code %2$d",
							locale = "en_UK", //
							explanation = "%1$s - terminated with exit-code %2$d" //
			), //
			@I18NMessage(
							value = "%1$s - Fehlercode %2$d wurde gesetzt",
							locale = "de", //
							explanation = "%1$s - terminated with exit-code %2$d" //
			)		//
					},
